...
```text
https://tubancavip.com/api/v1/store/sales
```
...
Envía únicamente la taquilla y las líneas seleccionadas. `amount` y `prize` se expresan en la unidad visible de la moneda, no en centavos. Por ejemplo, para vender `10.00`, envía `10`.
...
- Comisiones, moneda, agencia y total usando los datos actuales del servidor.
...
- Conserva el access token mediante el SDK oficial y renuévalo antes de caducar.
- El endpoint no incluye idempotencia. Cada `POST` es tratado como un nuevo ticket.